...
_trk = os.getenv("ULTRALYTICS_TRACKER", "bytetrack_surveillance.yaml")
TRACKER_CFG     = str(BASE_DIR / _trk) if (BASE_DIR / _trk).exists() else _trk
...
class DetectionPipeline:
    ...
    @staticmethod
    def _log_event(tid, direction, t):
        try:
            new = not EVENT_LOG.exists()
            with open(EVENT_LOG, "a") as f:
                if new:
                    f.write("timestamp,track_id,direction\n")
                f.write(f"{datetime.fromtimestamp(t).isoformat(timespec='seconds')},{tid},{direction}\n")
        except Exception as e:
            logger.warning(f"Could not write event log: {e}")
        logger.info(f"Person #{tid} -> {direction}")
        ...
        while self.running:
            ...
            try:
                ...
                if (results
                    ...
                    if boxes.id is not None:          # untracked boxes are not trustworthy for counting
                        xyxy  = boxes.xyxy.cpu().numpy().astype(int)
                        confs = boxes.conf.cpu().numpy()
                        ids   = boxes.id.cpu().numpy().astype(int)
                        min_h = MIN_BOX_H_FRAC * h
            ...
            with self.lock:
                live_inside, active_cnt = self.counter.update(detections, lp, invert=ln.get("invert", False))
                for d in detections:
    ...
    def _annotate(self, frame, detections, lp, invert, occ, ci, co, fps_val, anchor_mode, w, h):
        ...
        cv2.putText(ann, f"{ts}  [{w}x{h}]", (max(10, w - 290), max(20, h - 14)),
    ...
    def set_config(self, conf=None, anchor=None):
        with self.lock:
            ...
            if anchor is not None and anchor in ("head", "torso", "center", "feet"):
                self.anchor = anchor
